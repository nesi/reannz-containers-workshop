...
This workshop introduces **containers** and how to use them on a High Performance Cluster (HPC) such as [Mahuika](https://docs.nesi.org.nz/). Containers let you package software, along with all of its dependencies, into a single portable file that runs the same way on your own computer and on an HPC. By the end of the workshop you will be able to find, run, build, secure, and run parallel (MPI) containers using [Apptainer](https://apptainer.org/).

The material is split across two days: **Day 1: The Basics of Containers** (running and building containers) and **Day 2: Beyond the Basics** (securing and running MPI containers), with optional **supplementary** material.
...
| [5. Securing Containers with Encryption](05-securing-containers.md) | Encrypt a container with a passphrase or an RSA key pair |
| [6. Running Containers with MPI](06-running-MPI-containers.md) | Run containers in parallel with the hybrid and bind models |
| **Supplementary** | |
...
| [S5: Making Tests in Containers](S5-testing-containers.md) | Add a `%test` section and run `apptainer test` |
| [S6: Building a Container using Sandbox Mode](S6-building-a-container-using-sandbox-mode.md) | Build a container interactively while experimenting |
