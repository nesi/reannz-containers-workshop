...
* `%files`: copies files from the host into the container, written as `source destination` (we used this in [Chapter 9](09-running-MPI-containers.md) to copy programs into MPI containers).
...
* `%test`: commands run at the end of the build (and whenever you run `apptainer test`) to check that the container was built correctly (see [Chapter 8](08-testing-containers.md)).
* `%labels`: metadata such as the author and version, which you can read back with `apptainer inspect` (see [Chapter 4](04-building-images.md) and [Chapter 7](07-editing-containers.md)).
