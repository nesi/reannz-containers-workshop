...
## 5. Making Tests in Containers

From the [`05_making_tests_in_containers`](https://github.com/nesi/reannz-containers-workshop/tree/main/examples/05_making_tests_in_containers) directory:
...
From the [`07_running_containers_with_MPI`](https://github.com/nesi/reannz-containers-workshop/tree/main/examples/07_running_containers_with_MPI) directory. These containers build OpenMPI from source, which is CPU-intensive — it is best to build them through slurm. A few of them need an extra step.

The [**hybrid**](https://github.com/nesi/reannz-containers-workshop/blob/main/examples/07_running_containers_with_MPI/hybrid_model/mpi_hybrid_container.def) container builds everything itself, so you can build it directly:
...
The [**bind**](https://github.com/nesi/reannz-containers-workshop/blob/main/examples/07_running_containers_with_MPI/bind_model/mpi_bind_container.def) container needs the `mpi_hello_world` program compiled on the host first (it has no MPI of its own), then the container is built around it:
...
The [**OSU benchmark**](https://github.com/nesi/reannz-containers-workshop/tree/main/examples/07_running_containers_with_MPI/questions) containers (used in the exercises) each provide a `build_osu.sh` helper that performs any host-side compilation and then builds the container:
